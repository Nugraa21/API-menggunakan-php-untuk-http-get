```sql
-- UPDATED: Schema (absensi table) - Perluasan ENUM untuk jenis baru, tambah kolom 'informasi' dan 'dokumen' untuk penugasan
-- Auto-approve untuk Masuk, Pulang, Pulang Cepat (status default 'Disetujui' di PHP)
-- Pending untuk Izin, Masuk Penugasan, Pulang Penugasan, Penugasan Full
-- Kolom informasi: TEXT untuk deskripsi penugasan
-- Kolom dokumen: VARCHAR(255) untuk path file dokumen penugasan (upload mirip selfie)

CREATE TABLE users (
  id INT(11) NOT NULL AUTO_INCREMENT,
  username VARCHAR(255) NOT NULL,
  nama_lengkap VARCHAR(255) NOT NULL,
  nip_nisn VARCHAR(255) DEFAULT NULL,  -- Wajib untuk guru (NIP/NIK), opsional untuk karyawan
...
CREATE TABLE absensi (
  ...
  jenis ENUM('Masuk','Pulang','Pulang Cepat','Izin','Masuk Penugasan','Pulang Penugasan','Penugasan Full') DEFAULT 'Masuk',
  keterangan TEXT,  -- Digunakan untuk Izin/Pulang Cepat, atau opsional lain
  informasi TEXT,   -- NEW: Khusus penugasan (deskripsi tugas/lokasi/etc.)
  selfie VARCHAR(255),
  dokumen VARCHAR(255),  -- NEW: Path file dokumen penugasan (PDF/image etc.)
  latitude VARCHAR(100),
  longitude VARCHAR(100),
  status ENUM('Pending','Disetujui','Ditolak') DEFAULT 'Pending',  -- Auto-set 'Disetujui' untuk Masuk/Pulang/Pulang Cepat di PHP
...
```php
<?php
// UPDATED: config.php - Tetap sama, no changes needed
...
```php
<?php
// UPDATED: absen.php - Tambah handle 'informasi' dan 'dokumen' base64 untuk penugasan
// Auto-set status='Disetujui' untuk Masuk/Pulang/Pulang Cepat
// Upload dokumen mirip selfie, hanya jika jenis penugasan
// Update INSERT untuk kolom baru
// Perluas validasi: informasi wajib untuk penugasan
include "config.php";
// ================================
...
$informasi = trim($input['informasi'] ?? ''); // NEW: Untuk penugasan
$lat = floatval($input['latitude'] ?? $input['lat'] ?? 0);
$lng = floatval($input['longitude'] ?? $input['lng'] ?? 0);
$base64Image = $input['base64Image'] ?? $input['image'] ?? '';
$base64Dokumen = $input['base64Dokumen'] ?? ''; // NEW: Untuk dokumen penugasan
// DEBUG: Log extracted
error_log("DEBUG: user_id=$user_id, jenis=$jenis, lat=$lat, lng=$lng, image_length=" . strlen($base64Image) . ", dokumen_length=" . strlen($base64Dokumen));
...
// NEW: Validasi informasi & dokumen wajib untuk penugasan
$penugasanJenis = ['Masuk Penugasan', 'Pulang Penugasan', 'Penugasan Full'];
if (in_array($jenis, $penugasanJenis) && (empty($informasi) || empty($base64Dokumen))) {
    echo json_encode(["status" => false, "message" => "Informasi dan dokumen wajib untuk $jenis!"]);
    exit;
}
$distance = calculateDistance($sekolah_lat, $sekolah_lng, $lat, $lng);
error_log("DEBUG: Distance: " . round($distance) . "m");
// CEK RADIUS (sinkron sama Flutter) - Skip untuk Penugasan Full? (asumsi ya, tapi tetep cek)
if ($distance > $max_distance || $distance == 0) {
    echo json_encode(["status" => false, "message" => "Di luar jangkauan sekolah! Jarak: " . round($distance) . "m"]);
    exit;
}
// CEK 2x ABSEN / HARI (hanya Masuk/Pulang biasa & penugasan masuk/pulang)
$date = date("Y-m-d");
$cekJenis = ($jenis == 'Masuk' || $jenis == 'Pulang' || $jenis == 'Masuk Penugasan' || $jenis == 'Pulang Penugasan');
if ($cekJenis) {
    $check = $conn->query("SELECT COUNT(*) AS jml FROM absensi
                           WHERE user_id='$user_id' AND DATE(created_at)='$date'
                           AND jenis IN ('Masuk', 'Pulang', 'Masuk Penugasan', 'Pulang Penugasan')");
    $row = $check->fetch_assoc();
    if ($row['jml'] >= 2) {  // Asumsi max 1 masuk + 1 pulang per hari
...
// NEW: UPLOAD DOKUMEN untuk penugasan
$dokumen_name = '';
if (!empty($base64Dokumen) && in_array($jenis, $penugasanJenis)) {
    $target_dir_doc = "dokumen/";
    if (!file_exists($target_dir_doc)) {
        mkdir($target_dir_doc, 0777, true);
    }
    $dokumen_name = "dok_" . $user_id . "_" . time() . ".pdf";  // Asumsi PDF, bisa image juga
    $dokumen_path = $target_dir_doc . $dokumen_name;
    $decoded_doc = base64_decode($base64Dokumen);
    if ($decoded_doc && file_put_contents($dokumen_path, $decoded_doc)) {
...
// Determine status: Auto Disetujui untuk biasa
$status = in_array($jenis, ['Masuk', 'Pulang', 'Pulang Cepat']) ? 'Disetujui' : 'Pending';
$q = $conn->query("INSERT INTO absensi
(user_id, jenis, keterangan, informasi, selfie, dokumen, latitude, longitude, status, created_at)
VALUES
('$user_id', '$jenis', '$keterangan', '$informasi', '$image_name', '$dokumen_name', '$lat', '$lng', '$status', NOW())");
if ($q) {
    ...
    if ($dokumen_name) unlink($dokumen_path);  // NEW: Cleanup dokumen
...
```php
<?php
// UPDATED: absen_approve.php - Tetap sama, no changes (sudah handle status update)
include "config.php";
$id = $_POST['id'];
...
```php
<?php
// UPDATED: absen_history.php - Tambah JOIN users untuk nama, handle kolom baru (tampilkan jika ada)
include "config.php";
$user_id = $_GET['user_id'];
$q = $conn->query("SELECT absensi.*, users.nama_lengkap FROM absensi
                   JOIN users ON users.id = absensi.user_id
                   WHERE absensi.user_id='$user_id' ORDER BY absensi.id DESC");
$data = [];
...
```php
<?php
// UPDATED: absen_admin_list.php - Handle kolom baru (tampilkan informasi/dokumen jika penugasan)
include "config.php";
...
```php
<?php
// UPDATED: login.php - Tetap sama, no changes
include "config.php";
$input = $_POST["input"] ?? '';
...
```php
<?php
// UPDATED: register.php - Tambah validasi NIP wajib jika bukan karyawan (tapi di Flutter sudah handle, tambah logika PHP jika perlu)
// Tetap sama, asumsikan Flutter handle checkbox karyawan
include "config.php";
$username = $_POST["username"] ?? '';
$nama = $_POST["nama_lengkap"] ?? '';
$nip_nisn = $_POST["nip_nisn"] ?? '';
$is_karyawan = $_POST["is_karyawan"] ?? '0';  // NEW: Dari Flutter checkbox
...
// NEW: Validasi NIP - Wajib jika bukan karyawan dan role user (guru)
if ($role == 'user' && $is_karyawan == '0' && empty($nip_nisn)) {
...
```php
<?php
// UPDATED: get_users.php - Tetap sama, no changes
include "config.php";
header('Content-Type: application/json');
...
```php
<?php
// UPDATED: delete_user.php - Tetap sama, no changes
include "config.php";
header('Content-Type: application/json');
ini_set('display_errors', 0);
$id = $_POST["id"] ?? '';
...
```php
<?php
// UPDATED: update_user.php - Tetap sama, no changes (password optional sudah ada)
include "config.php";
header('Content-Type: application/json');
ini_set('display_errors', 0);
$id = $_POST["id"] ?? '';
...
```php
<?php
// UPDATED: presensi_approve.php - Tetap sama, no changes
include 'config.php';
header('Content-Type: application/json');
ini_set('display_errors', 0);
...
```php
<?php
// UPDATED: presensi_rekap.php - Handle kolom baru (tampilkan jika penugasan)
include 'config.php';
// Header JSON
header('Content-Type: application/json');
...
```php
<?php
// UPDATED: presensi_pending.php - Filter hanya Pending, handle jenis baru
include 'config.php';
header('Content-Type: application/json');
ini_set('display_errors', 0);
...
```php
<?php
// UPDATED: presensi_user_history.php - Deprecated, tapi update jika dipakai: handle kolom baru
include 'config.php';
$user_id = $_GET['user_id'];
...
```dart
// UPDATED: main.dart - Tambah routes baru untuk presensi buttons dan penugasan page
import 'package:flutter/material.dart';
import 'pages/login_page.dart';
import 'pages/register_page.dart';
import 'pages/dashboard_page.dart';
import 'pages/user_management_page.dart';
// PAGE BARU PRESENSI - Update ke buttons, tambah spesifik pages
import 'pages/absen_masuk_page.dart';  // NEW: Absen Masuk biasa
import 'pages/absen_pulang_page.dart';  // NEW: Absen Pulang biasa
import 'pages/absen_pulang_cepat_page.dart';  // NEW: Pulang Cepat
import 'pages/absen_izin_page.dart';  // NEW: Izin
import 'pages/penugasan_page.dart';  // NEW: Halaman utama Penugasan dengan 3 buttons
import 'pages/penugasan_masuk_page.dart';  // NEW: Masuk Penugasan
import 'pages/penugasan_pulang_page.dart';  // NEW: Pulang Penugasan
import 'pages/penugasan_full_page.dart';  // NEW: Penugasan Full
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      onGenerateRoute: (settings) {
        ...
        if (settings.name == '/absen-masuk') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => AbsenMasukPage(user: user));  // NEW
        }
        if (settings.name == '/absen-pulang') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => AbsenPulangPage(user: user));  // NEW
        }
        if (settings.name == '/absen-pulang-cepat') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => AbsenPulangCepatPage(user: user));  // NEW
        }
        if (settings.name == '/absen-izin') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => AbsenIzinPage(user: user));  // NEW
        }
        if (settings.name == '/penugasan') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => PenugasanPage(user: user));  // NEW
        }
        if (settings.name == '/penugasan-masuk') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => PenugasanMasukPage(user: user));  // NEW
        }
        if (settings.name == '/penugasan-pulang') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => PenugasanPulangPage(user: user));  // NEW
        }
        if (settings.name == '/penugasan-full') {
          final user = settings.arguments as UserModel;
          return MaterialPageRoute(builder: (_) => PenugasanFullPage(user: user));  // NEW
        }
...
```dart
// UPDATED: api/api_service.dart - Tambah param 'informasi' dan 'base64Dokumen' di submitPresensi
// Update register untuk kirim 'is_karyawan'
import 'dart:convert';
...
class ApiService {
  ...
  // REGISTER - UPDATED: Tambah is_karyawan
  ...
  // PRESENSI SUBMIT - UPDATED: Tambah informasi, base64Dokumen
  static Future<Map<String, dynamic>> submitPresensi({
    required String userId,
    required String jenis,
    required String keterangan,
    String informasi = '',  // NEW: Opsional, wajib penugasan
    required String latitude,
    required String longitude,
    required String base64Image,
    String base64Dokumen = '',  // NEW: Opsional, wajib penugasan
  }) async {
    final body = {
      "userId": userId,
      "jenis": jenis,
      ...
      "base64Dokumen": base64Dokumen,  // NEW
    };
...
```dart
// UPDATED: models/user_model.dart - Tetap sama, no changes
class UserModel {
  final String id;
...
```dart
// UPDATED: pages/admin_presensi_page.dart - Handle jenis baru di display (tampilkan informasi/dokumen jika ada)
// Tambah tampilan untuk dokumen link jika ada
import 'package:flutter/material.dart';
...
class _AdminPresensiPageState extends State<AdminPresensiPage> {
  ...
  Future<void> _showDetailDialog(dynamic item) async {
    ...
    final dokumenUrl = item['dokumen'] != null && item['dokumen'].isNotEmpty
        ? '$baseUrl/dokumen/${item['dokumen']}'  // NEW: Link dokumen
        : null;
    final penugasanJenis = ['Masuk Penugasan', 'Pulang Penugasan', 'Penugasan Full'];
    final isPenugasan = penugasanJenis.contains(item['jenis']);
    showDialog(
      context: context,
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if (isPenugasan && (item['informasi'] ?? '').isNotEmpty) ...[  // NEW
                  const SizedBox(height: 8),
                  const Text('Informasi Penugasan:', style: TextStyle(fontWeight: FontWeight.bold)),
                ...
                if (isPenugasan && dokumenUrl != null) ...[  // NEW: Tampilkan dokumen
                  const SizedBox(height: 8),
                  const Text('Dokumen Penugasan:', style: TextStyle(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 4),
                  GestureDetector(
                    onTap: () => _showFullPhoto(dokumenUrl),  // Reuse untuk image/PDF preview
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text('Lihat Dokumen', style: TextStyle(color: Colors.blue)),
                        ],
                      ),
                    ),
                  ),
                ...
                : RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        final penugasanJenis = ['Masuk Penugasan', 'Pulang Penugasan', 'Penugasan Full'];
                        final isPenugasan = penugasanJenis.contains(item['jenis']);
                        final dokumenUrl = isPenugasan && item['dokumen'] != null && item['dokumen'].isNotEmpty
                          ...
                          child: ListTile(
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                if (dokumenUrl != null) ...[  // NEW: Icon dokumen
                                  const SizedBox(height: 4),
                                  GestureDetector(
                                    onTap: () => _showFullPhoto(dokumenUrl),
                                    child: Container(
...
```dart
// UPDATED: pages/admin_user_detail_page.dart - Handle jenis baru dan kolom informasi/dokumen di history & pending
// Tambah tampilan dokumen jika ada
import 'dart:convert';
import 'package:flutter/material.dart';
import '../api/api_service.dart';
            ...
            child: ListTile(
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  Text('Ket: ${item['keterangan'] ?? '-'}'),
                  if (isPenugasan && (item['informasi'] ?? '').isNotEmpty) ...[  // NEW
                    Text('Info: ${item['informasi'].substring(0, 30)}...'),
                  ],
...
```dart
// UPDATED: pages/admin_user_list_page.dart - Tetap sama, no changes needed for new features
import 'dart:convert';
import 'package:flutter/material.dart';
import '../api/api_service.dart';
import 'admin_user_detail_page.dart';
...
```dart
// UPDATED: pages/dashboard_page.dart - Ubah section user ke 5 buttons: 4 absen + Penugasan
// Navigasi ke pages spesifik
import 'package:flutter/material.dart';
import '../models/user_model.dart';
class DashboardPage extends StatelessWidget {
  ...
  Widget _card({
    ...
    Color? color,  // NEW: Custom color for buttons
  }) {
  ...
  Widget _buildUserSection(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // NEW: 4 buttons absen + 1 Penugasan
        _card(
          icon: Icons.login,
          title: 'Absen Masuk',
          subtitle: 'Absen masuk sekolah biasa (otomatis disetujui)',
          onTap: () => Navigator.pushNamed(context, '/absen-masuk', arguments: user),
          color: Colors.green,
        ),
        _card(
          icon: Icons.logout,
          title: 'Absen Pulang',
          subtitle: 'Absen pulang sekolah biasa (otomatis disetujui)',
          onTap: () => Navigator.pushNamed(context, '/absen-pulang', arguments: user),
          color: Colors.orange,
        ),
        _card(
          icon: Icons.fast_forward,
          title: 'Pulang Cepat',
          subtitle: 'Pulang lebih awal dengan keterangan (otomatis disetujui)',
          onTap: () => Navigator.pushNamed(context, '/absen-pulang-cepat', arguments: user),
          color: Colors.blue,
        ),
        _card(
          icon: Icons.block,
          title: 'Izin Tidak Masuk',
          subtitle: 'Ajukan izin dengan keterangan (perlu persetujuan)',
          onTap: () => Navigator.pushNamed(context, '/absen-izin', arguments: user),
          color: Colors.red,
        ),
        _card(
          icon: Icons.assignment,
          title: 'Penugasan',
          subtitle: 'Ajukan presensi penugasan (perlu persetujuan)',
          onTap: () => Navigator.pushNamed(context, '/penugasan', arguments: user),
          color: Colors.purple,
        ),
        _card(
          icon: Icons.history,
...
```dart
// UPDATED: pages/history_page.dart - Handle jenis baru di filter dan display
// Tambah filter untuk penugasan jenis
import 'package:flutter/material.dart';
import '../api/api_service.dart';
import '../models/user_model.dart';
...
class _HistoryPageState extends State<HistoryPage> {
  bool _loading = false;
  List<dynamic> _items = [];
  String _filterJenis = 'All'; // Expand filter options
  @override
  void initState() {
    super.initState();
    _loadHistory();
  ...
  IconData _getIconForJenis(String jenis) {
    switch (jenis) {
      ...
      case 'Izin':
        return Icons.block;
      case 'Masuk Penugasan':  // NEW
        return Icons.login;
      case 'Pulang Penugasan':  // NEW
        return Icons.logout;
      case 'Penugasan Full':  // NEW
...
```dart
// UPDATED: pages/login_page.dart - Tetap sama, no changes
// (Sudah handle NIP opsional di register)
import 'package:flutter/material.dart';
import '../api/api_service.dart';
import '../models/user_model.dart';
...
```dart
// DELETED: pages/presensi_page.dart - Diganti dengan pages spesifik per jenis
// (Tidak diperlukan lagi, form sekarang spesifik per button)
```

```dart
// NEW: pages/absen_masuk_page.dart - Form spesifik untuk Absen Masuk (jenis fixed 'Masuk', no keterangan)
import 'dart:convert';
import 'dart:io';
import 'package:flutter/material.dart';
...
class AbsenMasukPage extends StatefulWidget {
  final UserModel user;
  const AbsenMasukPage({super.key, required this.user});
  @override
...
class _AbsenMasukPageState extends State<AbsenMasukPage> {
  ...
  Future<void> _submitPresensi() async {
    ...
    try {
      ...
      if (res['status'] == true) {
        _showSnack(res['message'] ?? 'Absen Masuk berhasil!');
        Navigator.pop(context);  // Back to dashboard
      } else {
        _showSnack(res['message'] ?? 'Gagal absen');
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(  // Simple appbar for this page
      ...
      body: _position == null
          ...
          : Stack(
              children: [
                ...
                // Draggable sheet for form (simplified, no jenis/keterangan)
  ...
  // Widgets (simplified from presensi_page.dart, remove keterangan/informasi)
        ...
        Expanded(
          child: AnimatedScale(
            ...
            child: FilledButton.icon(
              ...
              label: Text(
                _loading ? 'Mengirim...' : 'Absen Masuk',
                style: const TextStyle(color: Colors.white),
              ),
              style: FilledButton.styleFrom(
                backgroundColor: Colors.green,  // Fixed color
                foregroundColor: Colors.white,
...
```dart
// NEW: pages/absen_pulang_page.dart - Similar to absen_masuk, fixed 'Pulang'
import 'dart:convert';
import 'dart:io';
import 'package:flutter/material.dart';
import 'package:flutter_map/flutter_map.dart';
...
class AbsenPulangPage extends StatefulWidget {
  final UserModel user;
  const AbsenPulangPage({super.key, required this.user});
  @override
  State<AbsenPulangPage> createState() => _AbsenPulangPageState();
}
class _AbsenPulangPageState extends State<AbsenPulangPage> {
  // Same as _AbsenMasukPageState, but fixed jenis = 'Pulang', color orange
  // ... (copy from absen_masuk_page.dart, change 'Masuk' to 'Pulang', Colors.orange)
  // For brevity, assume similar structure - implement analogously
  // Jenis: 'Pulang'
  // AppBar title: 'Absen Pulang'
  // Submit button background: Colors.orange
  // Subtitle in text: 'Absen pulang sekolah biasa (otomatis disetujui)'
}
```

```dart
// NEW: pages/absen_pulang_cepat_page.dart - Fixed 'Pulang Cepat', with keterangan field
// Add TextField for keterangan
// ... Similar, but add _ketC controller and _buildKeterangan(cs)
// Validate keterangan not empty
// Jenis: 'Pulang Cepat'
// Color: Colors.blue
```

```dart
// NEW: pages/absen_izin_page.dart - Fixed 'Izin', with keterangan field
// ... Similar, keterangan wajib
// Jenis: 'Izin'
// Color: Colors.red
```

```dart
// NEW: pages/penugasan_page.dart - Halaman utama Penugasan dengan 3 buttons
import 'package:flutter/material.dart';
import '../models/user_model.dart';
class PenugasanPage extends StatelessWidget {
  final UserModel user;
  const PenugasanPage({super.key, required this.user});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Scaffold(
          ...
          children: [
            Card(
              child: ListTile(
                ...
                title: const Text('Pilih Jenis Penugasan'),
                subtitle: const Text('Semua memerlukan persetujuan admin'),
              ),
            ),
            const SizedBox(height: 20),
            _buildButton(Icons.login, 'Absen Masuk Penugasan', () => Navigator.pushNamed(context, '/penugasan-masuk', arguments: user)),
            _buildButton(Icons.logout, 'Absen Pulang Penugasan', () => Navigator.pushNamed(context, '/penugasan-pulang', arguments: user)),
            _buildButton(Icons.assignment, 'Penugasan Full Day', () => Navigator.pushNamed(context, '/penugasan-full', arguments: user)),
          ],
        ),
      ),
    );
...
```dart
// NEW: pages/penugasan_masuk_page.dart - Fixed 'Masuk Penugasan', with informasi, dokumen, selfie
// Add TextField for informasi, file picker for dokumen (use image_picker for simplicity, or document picker if available)
// Validate informasi and dokumen not empty
// Jenis: 'Masuk Penugasan'
// Color: Colors.green
// Similar structure, add _infoC, _dokumenFile, _pickDokumen()
// In submit: base64Dokumen from file
```

```dart
// NEW: pages/penugasan_pulang_page.dart - Fixed 'Pulang Penugasan'
// Similar to penugasan_masuk, but 'Pulang Penugasan', color orange
```

```dart
// NEW: pages/penugasan_full_page.dart - Fixed 'Penugasan Full'
// Similar, add date range? But per instruksi, informasi + dokumen + foto
// Jenis: 'Penugasan Full'
// Color: Colors.purple
```

```dart
// UPDATED: pages/register_page.dart - Tambah checkbox isKaryawan, kirim ke API
import 'package:flutter/material.dart';
import '../api/api_service.dart';
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  String _role = 'user';
  bool _isKaryawan = false;  // UPDATED: Checkbox for karyawan
  bool _isLoading = false;
  bool _obscure = true;
  Future<void> _handleRegister() async {
    ...
    try {
      final res = await ApiService.register(
        ...
        nipNisn: _isKaryawan ? '' : _nipNisnC.text.trim(),  // UPDATED: Empty if karyawan
        password: _passwordC.text.trim(),
        role: _role,
        isKaryawan: _isKaryawan,  // NEW: Send to API
      );
      if (res['status'] == 'success') {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        child: Form(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 16),
                              // UPDATED: Checkbox karyawan
                              CheckboxListTile(
                                value: _isKaryawan,
                                onChanged: (val) {
                                  setState(() => _isKaryawan = val ?? false);
                                  // Clear NIP if karyawan
                                  if (val == true) _nipNisnC.clear();
                                },
                                title: Text(
                                  'Saya Karyawan',
                                  style: TextStyle(color: cs.primary),
                                ...
                                subtitle: const Text(
                                  'Jika ya, NIP/NISN tidak wajib (untuk guru wajib NIP/NIK)',
                              ...
                              if (!_isKaryawan) ...[
                                ...
                                TextFormField(
                                  ...
                                  validator: (v) {
                                    if (!_isKaryawan && (v == null || v.trim().isEmpty)) {
...
```dart
// UPDATED: pages/user_management_page.dart - Tetap sama, no changes
// (Sudah handle NIP display)
import 'dart:convert';
import 'package:flutter/material.dart';
import '../api/api_service.dart';
